...
class ReadScreenTool(BaseAndroidTool):
    ...
    def description(self) -> str:
        return (
            "Dumps the current Android UI hierarchy and returns a clean list of interactive elements. "
    ...
    async def run(self, **kwargs) -> ActionResult:
        try:
            ...
            if not parsed_ui:
                return ActionResult(
                    success=False, 
                    message="Failed to parse UI. Screen might be locked or empty."
                )

            return ActionResult(
                success=True,
                message=f"Successfully extracted {len(parsed_ui)} interactive elements from the screen.",
            ...
            return ActionResult(
                success=False, 
                message=f"Error reading screen: {str(e)}"
            )